...
Future maybeCreateUser(User user) async {
  final userRecord = BookingsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await BookingsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createBookingsRecordData(
    email: user.email,
  ...
  currentUserDocument =
      BookingsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createBookingsRecordData(email: email));
}
